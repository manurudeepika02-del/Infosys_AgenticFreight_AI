...
🏷️ Badges
Python Streamlit SQLite Scikit-Learn Hugging Face LangChain FAISS Machine Learning AI RAG GitHub License
...
🔄 Project Workflow
User Authentication: User logs into the platform via credential validation or email OTP verification.
Access Control: Authentication module validates credentials and issues a secure user session.
Module Selection: User navigates the unified dashboard to select specific operational AI tools.
Freight Price Estimation: Machine learning agents predict quotation prices based on cargo inputs.
Route & Environmental Assessment: Route and weather module evaluates shipping lane conditions.
Carrier Auditing: Carrier Audit agent evaluates performance, compliance score, and reliability.
RAG Context Retrieval: RAG pipeline searches the vector database for relevant logistics documentation chunks.
AI Copilot Synthesis: AI Copilot combines retrieved document context and ML predictions into grounded responses.
Business Intelligence: Analytics dashboard displays real-time metrics, quotation histories, and model performance.
Administrative Controls: Admin users manage system accounts, audit logs, and trigger model retraining.
📁 Repository Structure
...
Document Ingestion: Loading PDF files containing shipping rules, port operations, and compliance policies.
Text Preprocessing & Chunking: Splitting documents into overlapping text chunks to preserve semantic context.
Embedding Generation: Converting text chunks into high-dimensional dense vector representations.
Vector Storage: Indexing vectors inside a high-performance FAISS index for similarity search.
Context Retrieval: Performing k-Nearest Neighbors (k-NN) search to retrieve the most relevant text chunks for a query.
Augmented Generation: Formatting retrieved chunks into an explicit context prompt for the Qwen 2.5 model.
🔐 Security Features
...
📖 User Guide
Account Registration: Navigate to the registration page, enter your user details, and set up security questions.
OTP Verification: Enter the One-Time Password sent to your registered email address to activate your account.
Secure Login: Sign in with your credentials to generate a secure session token.
Dashboard Access: Select your operational goal from the main navigation panel.
Generate Freight Quotations: Input shipment origin, destination, container type, and weight into the Freight Pricing module to get instant ML cost estimates.
Analyze Route & Weather: Run route simulations to check transit risks and current ocean weather alerts.
Audit Carrier Reliability: Query the Carrier Audit database to compare carrier scores and compliance records.
Query RAG Knowledge Base: Use the RAG interface to search logistics documentation and ask domain-specific questions to the AI Copilot.
Admin Administration: Log in with administrator credentials to manage user access, review system audit logs, or trigger model retraining.
📸 Application Screenshots
Login
Login
...
Repository Cloning: Each team member created a local clone of the shared central repository.
Feature Branching: Development was isolated on task-specific branches off main (feature/auth, feature/rag-pipeline).
Commit Standards: Modular commits accompanied by descriptive commit messages.
Pull Requests (PRs): Work was submitted for integration via detailed GitHub Pull Requests.
Peer Review: Main maintainers reviewed code changes, verified tests, and managed branch merges.
📌 Repository Guidelines
